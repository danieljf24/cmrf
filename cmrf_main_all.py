...
from devise import Devise_pre
from model.devise_model import Devise

from psi import PSI_pre
...
def process(options, trainCollection, devCollection):
    ...
    # result path
    if method == 'devise':
        ranking_result_path =  os.path.join(rootpath, devCollection, 'SimilarityIndex', devCollection, 'MetaData', method, feature)#, options.devise_model_name)
        DCG_result_path = os.path.join(rootpath, devCollection, metric, method, feature)#, options.devise_model_name)
    elif method == 'psi':
        ranking_result_path =  os.path.join(rootpath, devCollection, 'SimilarityIndex', devCollection, 'MetaData', method, feature)#, options.psi_model_name)
        DCG_result_path = os.path.join(rootpath, devCollection, metric, method, feature)#, options.psi_model_name) 
    else:
    ...
    if method =='conse':
        ...
        # devise_model_path = os.path.join(rootpath, trainCollection , 'cv', 'cv_devise_model.py', 
        #     "flickr_vec200flickr25m_" + feature, options.learning_rate, options.devise_model_name)
        devise_model_path = os.path.join('data','trained_models', feature, 'devise', options.devise_model_name)
        devise_searcher = Devise_pre(devise_model_path, corpus, word2vec_model, dev_feat_path, rootpath)

    elif method == 'psi':
        # psi_model_path = os.path.join(rootpath, trainCollection , 'cv', 'cv_psi_model.py', 
        #     "flickr_vec200flickr25m_" + feature, options.learning_rate, options.psi_model_name)
        psi_model_path = os.path.join('data','trained_models', feature, 'psi', options.psi_model_name)
        psi_bow_path = os.path.join('data','bow_models', options.psi_name)
        psi_searcher = PSI_pre(psi_model_path, psi_bow_path, dev_feat_path)
    ...
    for query_id in qid_list:
        ...
        iid_label = zip(iid_list, label_list)
        random.shuffle(iid_label)
        iid_list = [x[0] for x in iid_label]
        label_list = [x[1] for x in iid_label]   
        ...
        if method == 'conse':
            ...
            scorelist = devise_searcher.predict_score(qid2query[query_id], iid_list)

        elif method == 'psi':
            scorelist = psi_searcher.predict_score(qid2query[query_id], iid_list)
...
def main(argv=None):
    ...
    parser.add_option("--method", default='conse', type="string", help="method:conse, t2i(ta), i2t(ia), pso, devise  (default: conse)")
    ...
    # theano model: Devise and PSI
    parser.add_option("--psi_name", default='msr2013train_query_bow_top50k.pkl', type="string", help="psi bow model name")
    # parser.add_option("--devise_model_path", default='trained_models/devise/trained_devise_model.pkl', type="string", help="trained devise model name")
    # parser.add_option("--psi_model_path", default='trained_models/psi/trained_psi_model.pkl', type="string", help="trained psi model name")

    parser.add_option('--learning_rate', type="float", default=0.01, help='solver learning rate' )    
    parser.add_option("--devise_model_name", default='trained_devise_model.pkl', type="string", help="devise model name")
    parser.add_option("--psi_model_name", default='trained_psi_model.pkl', type="string", help="psi model name")
