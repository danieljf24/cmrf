...
DEFAULT_WORD2VEC = 'vec200flickr25m.L2'
# DEFAULT_CORPUS = 'clickturefull'
# DEFAULT_WORD2VEC = 'w2v-full'
...
class Devise(object):
    ...
    def __init__(
        ...
        if not W_x:
            ...
            initial_W = numpy.asarray(
                numpy_rng.uniform(
                    low=-4 * numpy.sqrt(6. / (n_img + n_query)),
                    high=4 * numpy.sqrt(6. / (n_img + n_query)),
                    size=(n_query, n_img)
                ),
    ...
    def get_cost(self):
        plus_vector = self.predict_score_batch(self.query, self.plus)
        minus_vector = self.predict_score_batch(self.query, self.minus)
...
class DataSet():
    ...
    def getBatchData(self, index, qry_feats, img_feats):
        ...
        for qry in query_list:
            query.append(feats[renamed.index(qry)])
        
        # relevant images, irrelevant image, query of bag-of-words
...
def process(options, collection):
    ...
    # query2vec
    # query_feat_path = os.path.join(rootpath, collection, 'QueryFeatureData', query_feature)
    query_feat_path = os.path.join(rootpath, collection, 'QueryFeatureData', 'mincc_2_maximg_20', 'filtered.'+embedding+"_"+word2vec)
    query_feats = BigFile(query_feat_path)
    ...
    # dataset 
    train_file = os.path.join(rootpath, collection, 'TextData', 'triplets', 'mincc_2_maximg_20', 'train.triplet.random.txt')
    val_file = os.path.join(rootpath, collection, 'TextData', 'triplets',  'mincc_2_maximg_20','val.triplet.random.txt')

    # log file
    model_file = os.path.join(rootpath, collection, 'cv' ,checkpoint_output_dir,  
         query_feature + '_' + img_feature, 'lr_%.4f' % initial_learning_rate)
    ...
    if init_model != '':
        devise_init = cPickle.load(open(init_model, 'rb'))
        devise = Devise(
            numpy_rng=rng,
            input_plus=x,
            input_minus=y,
    ...
    for gparam_mom, gparam in zip(gparams_mom, gparams):
        ...
        # added by daniel     sgd with momentum
        # updates[gparam_mom] = mom * gparam_mom - learning_rate * gparam

    # ... and take a step along that direction
    for param, gparam_mom in zip(devise.params, gparams_mom):
        # Misha Denil's original version
        #stepped_param = param - learning_rate * updates[gparam_mom]
        
        # since we have included learning_rate in gparam_mom, we don't need it here
    ...
    validate_devise = theano.function(
        [newX, newY, newQ],
        cost,
        givens={
            x : newX,
    ...
    while (epoch_counter < max_epochs):
        ...
        for minibatch_index in index_list:
            new_train_X, new_train_Y, new_train_Q = trainData.getBatchData(minibatch_index, query_feats, img_feats)
            training_losses.append(train_devise(new_train_X, new_train_Y, new_train_Q, epoch_counter))
        ...
        # for i in range(10):
            new_val_X, new_val_Y, new_val_Q = validData.getBatchData(i, query_feats, img_feats)
            validation_losses.append(validate_devise(new_val_X, new_val_Y, new_val_Q))
        ...
        if this_validation_loss < best_validation_loss:
            ...
            # write out the parameters of devise modle
            fout_file = os.path.join(model_file, 'devise_lr_%.6f_iter_%d_loss_%.3f.pkl' % ( learning_rate.get_value(), best_iter, best_validation_loss))                  
            fout = file(fout_file, 'wb')
            cPickle.dump(devise, fout, -1)
            fout.close()
        else:
...
def main(argv=None):
    ...
    parser = OptionParser(usage="""usage: %prog [options] collection""")   #msr2013train
    ...
    # model parameters
    parser.add_option('--n_query', type="int", default=200, help='the number of query input neurons')
